{
  ...
  outputs = { self, unpins-lib }:
    let
      ...
      xmlMan = pkgsX.runCommand "xmllint-man"
        { nativeBuildInputs = [ pkgsX.libxslt pkgsX.docbook_xsl_ns ]; }
        ''
          mkdir -p "$out/share/man/man1"
          for p in xmllint xmlcatalog; do
            xsltproc --nonet --novalid --param man.output.quietly 1 \
              -o "$out/share/man/man1/$p.1" \
              ${pkgsX.docbook_xsl_ns}/xml/xsl/docbook/manpages/docbook.xsl \
              ${pkgsX.libxml2.src}/doc/$p.xml
          done
        '';
    in
    ulib.mkStandaloneFlake {
      ...
      build = pkgs:
        import ./multicall.nix { lib = pkgs.lib // ulib; }
          { inherit pkgs xmlMan; xml = pkgs.pkgsStatic.libxml2; };
      windowsBuild = pkgs:
        import ./multicall.nix { lib = pkgs.lib // ulib; }
          { inherit pkgs xmlMan; xml = (ulib.mingwStaticCross pkgs).libxml2; };
    };
}
